...
const imgui = @This();
pub const DrawCallback_ResetRenderState = @as(DrawCallback, @ptrFromInt(~@as(usize, 0)));
...
pub const FLT_MAX: f32 = @as(f32, @bitCast(@as(u32, 0x7F7FFFFF)));
pub const FLT_MIN: f32 = @as(f32, @bitCast(@as(u32, 0x00800000)));
...
pub fn Vector(comptime T: type) type {
    return extern struct {
        ...
        pub fn deinit(self: *@This()) void {
            if (self.Data) |d| raw.igMemFree(@as(*anyopaque, @ptrCast(d)));
            self.* = undefined;
        }
        ...
        pub fn clone(self: @This()) @This() {
            ...
            if (self.Size != 0) {
                ...
                // , self.Size * @sizeOf(T)
                @memcpy(@as([*]u8, @ptrCast(cloned.Data.?)), @as([*]const u8, @ptrCast( self.Data.?)));
            }
            return cloned;
        }
        ...
        pub fn copy(self: *@This(), other: @This()) void {
            ...
            if (other.Size != 0) {
                ...
                //, other.Size * @sizeOf(T)
                @memcpy(@as([*]u8, @ptrCast( self.Data.?)), @as([*]const u8, @ptrCast( other.Data.?)));
            }
        }
        ...
        pub fn from_slice(slice: []const T) @This() {
            ...
            if (slice.len != 0) {
                ...
                //, slice.len * @sizeOf(T)
                @memcpy(@as([*]u8, @ptrCast( result.Data.?)), @as([*]const u8, @ptrCast(slice.ptr)));
            }
            return result;
        }
        ...
        pub fn clear_delete(self: *@This()) void {
            ...
            if (is_optional) {
                ...
                for (self.items()) |_ptr| {
                    const ptr: *ValueT = _ptr;
                    destruct(ValueT, ptr);
                    raw.igMemFree(@as(?*anyopaque, @ptrCast(ptr)));
                }
            }
            self.clear();
        }
